jQuery(document).ready(function () {
	const form = jQuery('#abf_add_shipping_rule');
	if (form) {
		form.on('submit', enAbfSaveShippingRule);
	}

	// #region: Add Shipping Rule
	jQuery('#en_abf_add_shipping_rule_btn').on('click', function () {
		form[0]?.reset();
		...
		enAbfToggleRuleTypeFields();
	});

	jQuery('#en_abf_sr_rule_type').on('load change', function (e) {
		enAbfToggleRuleTypeFields();
	});
	...
	jQuery(document).on('click', '.en_abf_sr_edit_link', function () {
		const id = jQuery(this).attr('data-id');
		enAbfEditShippingRule(id, this);
	});
	jQuery(document).on('click', '.en_abf_sr_delete_link', function () {
		const id = jQuery(this).attr('data-id');
		enAbfDeleteShippingRule(id, this);
	});
	jQuery(document).on('click', '.en_abf_sr_status_link', function (e) {
		enAbfUpdateShippingRuleStatus(this);
	});
	...
	function enAbfSRValidateInput(form_id = '') {
		let is_valid = true;
		...
		if (jQuery('#en_abf_sr_rule_type').val() === 'Override Rates') {
	...
	function enAbfSaveShippingRule(e) {
		e.preventDefault();

		const is_valid = enAbfSRValidateInput('#abf_add_shipping_rule');
		if (!is_valid) {
		...
		const postData = {
			action: 'en_abf_save_shipping_rule',
			rule_id: jQuery('#edit_sr_form_id').val(),
			name: jQuery('#en_sr_rule_name').val(),
			type: jQuery('#en_abf_sr_rule_type').val(),
		...
		jQuery.ajax({
			...
			dataType: 'json',
			beforeSend: function () {
				jQuery('.en_abf_save_shipping_rule_form').addClass('spinner_disable').val('Loading...');
			},
			success: function (data) {
				jQuery('.en_abf_save_shipping_rule_form').removeClass('spinner_disable').val('Save');
        const rowMarkup = enAbfGetRowMarkup({ ...data, ...postData });
        jQuery('.en_abf_empty_sr_row').hide();

				if (data.insert_qry == 1) {
					const newRowMarkup = `<tr id='sr_row_${data.id}' class="en_abf_sr_row">${rowMarkup}</tr>`;
					jQuery('#en_shipping_rules_list tbody').append(newRowMarkup);
					window.location.href = jQuery('.close').attr('href');
					jQuery('.abf_sr_created').show('slow').delay(3000).hide('slow');
				} else if (data.update_qry == 1) {
					jQuery(`#sr_row_${data.id}`).html(rowMarkup);
					window.location.href = jQuery('.close').attr('href');
					jQuery('.abf_sr_updated').show('slow').delay(3000).hide('slow');
				} else if (data.error) {
					jQuery('.abf_sr_already_exist').show('slow');
					jQuery('.content').delay(200).animate({ scrollTop: 0 }, 300);
					setTimeout(function () {
						jQuery('.abf_sr_already_exist').hide('slow');
					}, 3000);
				}
			},
	...
	if (typeof enAbfEditShippingRule != 'function') {
		function enAbfEditShippingRule(rule_id, elem) {
			form[0]?.reset();
			jQuery('.en_sr_err').html('');
			setDataAttribute(
				'en_sr_weight_from',
			...
			const postForm = {
				action: 'en_abf_edit_shipping_rule',
				edit_id: rule_id,
			};
			...
			jQuery.ajax({
				...
				success: function (data) {
					...
					if (data) {
						...
						jQuery('#en_abf_sr_rule_type').val(data?.rule_data?.type);
						...
						// Show popup modal
						window.location.href = jQuery('.en_abf_add_sr_btn').attr('href');
						setTimeout(function () {
							if (jQuery('.en_abf_add_sr_popup').is(':visible')) {
	...
	if (typeof enAbfDeleteShippingRule != 'function') {
		function enAbfDeleteShippingRule(id, elem) {
			const postForm = {
				action: 'en_abf_delete_shipping_rule',
				delete_id: id,
			};
          ...
          jQuery('.abf_sr_deleted').show('slow').delay(3000).hide('slow');
          if (jQuery('.en_abf_sr_row')?.length) return false;

          if (jQuery('.en_abf_empty_sr_row')?.length) {
						jQuery('.en_abf_empty_sr_row').show();
					} else {
						const newRowMarkup = `<tr class="new_warehouse_add en_abf_empty_sr_row" data-id=0><td class="en_wd_warehouse_list_data" colspan="5" style="text-align: center;">No data found!</td></tr>`;
						jQuery('#en_shipping_rules_list tbody').append(newRowMarkup);
					}
				},
				error: function (error) {},
	...
	function enAbfUpdateShippingRuleStatus(elem) {
		const rule_id = jQuery(elem).attr('data-id');
		const is_active = jQuery(elem).attr('data-status');

		// Submit the form to save settings
		const postData = {
			action: 'en_abf_update_shipping_rule_status',
			rule_id,
	...
	function enAbfGetRowMarkup(rule) {
		const rowMarkup = `
	...
	function enAbfToggleRuleTypeFields() {
		const type = jQuery('#en_abf_sr_rule_type').val();
		jQuery('#en_abf_sr_content').css('height', '80vh');
		...
		if (type === 'Hide Methods') {
			...
			jQuery('#abf_add_shipping_rule').css('overflow', 'hidden');
			jQuery('#en_abf_sr_content').css('height', '450px');
