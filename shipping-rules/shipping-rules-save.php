...
if (!class_exists("EnAbfShippingRulesAjaxReq")) {

    class EnAbfShippingRulesAjaxReq
    {
        /**
        ...
        {
            add_action('wp_ajax_nopriv_en_abf_save_shipping_rule', array($this, 'en_abf_save_shipping_rule_ajax'));
            add_action('wp_ajax_en_abf_save_shipping_rule', array($this, 'en_abf_save_shipping_rule_ajax'));

            add_action('wp_ajax_nopriv_en_abf_edit_shipping_rule', array($this, 'en_abf_edit_shipping_rule_ajax'));
            add_action('wp_ajax_en_abf_edit_shipping_rule', array($this, 'en_abf_edit_shipping_rule_ajax'));

            add_action('wp_ajax_nopriv_en_abf_delete_shipping_rule', array($this, 'en_abf_delete_shipping_rule_ajax'));
            add_action('wp_ajax_en_abf_delete_shipping_rule', array($this, 'en_abf_delete_shipping_rule_ajax'));

            add_action('wp_ajax_nopriv_en_abf_update_shipping_rule_status', array($this, 'en_abf_update_shipping_rule_status_ajax'));
            add_action('wp_ajax_en_abf_update_shipping_rule_status', array($this, 'en_abf_update_shipping_rule_status_ajax'));
        }
         ...
         * @global $wpdb
         */
        function en_abf_save_shipping_rule_ajax()
        {
            global $wpdb;
            ...
            $qry = "SELECT * FROM " . $wpdb->prefix . "eniture_abf_shipping_rules WHERE name = '" . $data['name'] . "'"; 
            ...
            if (!empty($get_shipping_rule_id)) {
                ...
                $update_qry = $wpdb->update(
                    $wpdb->prefix . 'eniture_abf_shipping_rules', $data, array('id' => $get_shipping_rule_id)
                );
                ...
                if (!empty($get_shipping_rule)) {
                    ...
                    $data['settings'] = json_encode($data['settings']);
                    $insert_qry = $wpdb->insert($wpdb->prefix . 'eniture_abf_shipping_rules', $data);
         ...
         * @global $wpdb
         */
        function en_abf_edit_shipping_rule_ajax()
        {
            global $wpdb;
            $get_shipping_rule_id = (isset($_POST['edit_id']) && intval($_POST['edit_id'])) ? $_POST['edit_id'] : "";
            $shipping_rules_list = $wpdb->get_results(
                "SELECT * FROM " . $wpdb->prefix . "eniture_abf_shipping_rules WHERE id=$get_shipping_rule_id"
            );
            $product_tags_markup = $this->en_abf_get_product_tags_markup($shipping_rules_list);
         ...
         * @global $wpdb
         */
        function en_abf_delete_shipping_rule_ajax()
        {
            global $wpdb;
            $get_shipping_rule_id = (isset($_POST['delete_id']) && intval($_POST['delete_id'])) ? $_POST['delete_id'] : "";
            $qry = $wpdb->delete($wpdb->prefix . 'eniture_abf_shipping_rules', array('id' => $get_shipping_rule_id));
         ...
         * @global $wpdb
         */
        function en_abf_update_shipping_rule_status_ajax()
        {
            global $wpdb;
         ...
         * @global $wpdb
         */
        function en_abf_get_product_tags_markup($shipping_rules_list)
        {
            $tags_options = '';
            ...
            $selected_tags_detials = $this->en_abf_get_selected_tags_details($tags_data['filter_by_product_tag_value']);
         ...
         * @global $wpdb
         */
        function en_abf_get_selected_tags_details($products_tags_arr)
        {
         ...
         *
         * @param array $abf_package request to get quotes
         ...
         */
        function apply_shipping_rules($abf_package, $apply_on_rates = false, $rates = [], $loc_id = '')
        {
            if (empty($abf_package)) return $apply_on_rates ? $rates : false;

            global $wpdb;
            $qry = "SELECT * FROM " . $wpdb->prefix . "eniture_abf_shipping_rules"; 
            ...
            foreach ($rules as $rule) {
                ...
                if ($rule_type == 'Override Rates' && $apply_on_rates) {
                    $rates = $this->apply_override_rates_rule($abf_package, $settings, $rates, $loc_id);
                } else if ($rule_type == 'Hide Methods') {
                    $is_rule_applied = $this->apply_hide_methods_rule($settings, $abf_package);
         ...
         * @param array $abf_package The package to apply the rule to.
         * @return bool Whether the rule was applied or not.
        */
        function apply_hide_methods_rule($settings, $abf_package)
        {
            $is_rule_applied = false;

            if ($settings['apply_to'] == 'cart') {
                $formatted_values = $this->get_formatted_values($abf_package);
                $is_rule_applied = $this->apply_rule_filters($settings, $formatted_values);
            } else {
                foreach ($abf_package as $key => $pkg) {
                    $is_rule_applied = false;
                    $shipments = [];
                    $shipments[$key] = $pkg;
         ...
         *
         * @param array $abf_package request array to get the quotes
         * @param array $settings rule settings
         * @param object $rates quotes object
         * @return $rates The updated rates.
         */
        function apply_override_rates_rule($abf_package, $settings, $rates, $loc_id)
        {
        ...
        function get_updated_rates($rates, $settings)
        {
            ...
            if ($service_type == 'transportation_service') {
                ...
                $rates = $this->get_resi_or_lfg_service_rates($rates, $service_rate, 'RESIDENTIALDELIVERY');
            } elseif ($service_type == 'liftgate_delivery_service' && $liftgate_status) {
                $rates = $this->get_resi_or_lfg_service_rates($rates, $service_rate, 'LIFTGATEGROUNDDELIVERY');
            }
        ...
        function get_transportation_service_rates($rates, $service_rate)
        {
            ...
            foreach ($_rates as $key) {
                if (!isset($rates->{$key}) || !empty($rates->{$key}->ERROR)) continue;

                $quotes = $rates->{$key};
                if (empty($quotes) || !isset($quotes->CHARGE)) continue;
                
                $surcharges = isset($quotes->INCLUDEDCHARGES) ? $quotes->INCLUDEDCHARGES : '';
                $quotes->CHARGE = floatval($service_rate) + $this->get_resi_and_lfg_fees($surcharges);
        ...
        {
            $_rates = ['q', 'quotesWithoutLiftgate'];
            $surcharges_types = ['RESIDENTIALDELIVERY', 'LIFTGATEGROUNDDELIVERY'];
            ...
            foreach ($_rates as $key) {
                ...
                foreach ($surcharges_types as $s_type) {
                    if ($s_type != $type || !isset($surcharges->$s_type) || empty((array) $surcharges->$s_type)) continue;

                    $surcharges_fee = $surcharges->$s_type;
                    $quotes->INCLUDEDCHARGES->$s_type = $service_rate;
                ...
                $rate_charge = isset($quotes->CHARGE) ? $quotes->CHARGE : 0;
                $rate_charge = floatval($rate_charge) - floatval($surcharges_fee);
                $rate_charge += floatval($service_rate);
                isset($quotes->CHARGE) ? $quotes->CHARGE = $rate_charge : null;
                $rates->{$key} = $quotes;
            }
        ...
        function get_resi_and_lfg_fees($surcharges)
        {
            ...
            foreach ($surcharges_types as $s_charge) {
                if (isset($surcharges->$s_charge) && !empty($surcharges->$s_charge) && $surcharges->$s_charge > 0 && !empty((array) $surcharges->$s_charge)) {
