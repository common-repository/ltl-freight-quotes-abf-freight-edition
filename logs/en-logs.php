...
if (!class_exists('EnAbfLfqLogs')) {
    class EnAbfLfqLogs
    {
        ...
        {
            $obj_classs = new \ABF_Curl_Request();
            $data = array(
                'serverName' => abf_freight_get_domain(),
                'licenseKey' => get_option('wc_settings_abf_plugin_licence_key'),
                'lastLogs' => '25',
                'carrierName' => 'Abf'
            );
            ...
            $url = ABF_DOMAIN_HITTING_URL . '/request-log/index.php';
            $logs = $obj_classs->abf_get_curl_response($url, $data);
            ...
            if (isset($logs['severity'], $logs['data']) && $logs['severity'] == 'SUCCESS') {
                ...
                foreach ($logs['data'] as $key => $shipment) {
                    ...
                    $class_name = 'abf-log-' . $key . rand(1, 100);
